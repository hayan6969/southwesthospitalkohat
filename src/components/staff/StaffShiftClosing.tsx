...
export function StaffShiftClosing() {
  ...
  return (
    <div className="space-y-6">
      {/* No shift assigned warning */}
      {!(profile as any)?.shift && (
        <Card className="border-amber-300 bg-amber-50">
          <CardContent className="py-4">
            <div className="flex items-center gap-2 text-amber-700">
              <AlertTriangle className="w-4 h-4" />
              <p className="text-sm font-medium">No shift assigned to your account. Ask an admin to assign your shift. Showing full day revenue as fallback.</p>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-lg">
                {staffShift.toLowerCase() === 'morning' ? '🌅 Morning' : staffShift.toLowerCase() === 'evening' ? '🌆 Evening' : `⏰ ${staffShift}`} Shift Closing
              </CardTitle>
            </div>
            <Badge variant={staffShift.toLowerCase() === 'morning' ? 'default' : 'secondary'}>
              {staffShift} Shift
            </Badge>
          </div>

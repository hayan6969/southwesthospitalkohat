-- Update the calculate_doctor_earnings function to exclude free appointments (cleared_at is not null)
...
DECLARE
  ...
  total_earn NUMERIC;
BEGIN
  -- Count completed appointments in the period (excluding free appointments)
  SELECT COUNT(*) INTO appointment_cnt
  FROM public.appointments a
  WHERE a.doctor_id = p_doctor_id
    AND a.status = 'completed'
    AND a.cleared_at IS NULL  -- Exclude free appointments
  ...
  -- Calculate consultation earnings using historical consultation fees (excluding free appointments)

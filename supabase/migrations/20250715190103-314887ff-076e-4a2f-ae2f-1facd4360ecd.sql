-- Update the delete_user_safely function to fix type casting issues
...
BEGIN
  ...
  -- Delete audit logs (user_id is text, so cast uuid to text)
  DELETE FROM public.audit_logs WHERE user_id = user_uuid::text;
